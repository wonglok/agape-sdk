...
// import { NoodleO3 } from "./Noodle/NoodleO3";
        ...
        decorate={function decorate({ glb }) {
          ...
          glb.scene.traverse((it) => {
            if (it.isMesh) {
              ...
              unitGeomtry.applyMatrix4(it.matrixWorld);
            }
          });
...
export function CoreEngine({
  ...
  useEffect(() => {
    ...
    let bufferAlpha = !useHalfFloat
      ? new Float32Array(sceneDataAlpha)
      : new Uint16Array(sceneDataAlpha);

    let bufferBeta = !useHalfFloat
      ? new Float32Array(sceneDataBeta)
      : new Uint16Array(sceneDataBeta);
    ...
    let syncData = () => {
      ...
      for (let [key, object] of entries) {
        ...
        // 2 ===
        sceneDataTextureBeta.image.data[i * 4 + 0] = getData(
          getForceType(object?.userData?.forceType)
        );
        sceneDataTextureBeta.image.data[i * 4 + 1] = getData(
          getForceSize(object?.userData?.forceSize)
        );

        //
        sceneDataTextureBeta.image.data[i * 4 + 2] = getData(
          getForceTwist(object?.userData?.forceTwist)
        );
    ...
    // gpu.setVariableDependencies(iVar.velocity, [iVar.velocity]);
    ...
    renderMaterial.onBeforeCompile = (shader, gl) => {
      let sync = () => {
        ...
        //
        shader.uniforms.iv_position.value = gpu.getCurrentRenderTarget(
          iVar.position
        ).texture;
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        transformed.xyz *= rotation3dX(posData.x);
        transformed.xyz *= rotation3dY(posData.y);
        transformed.xyz *= rotation3dZ(posData.z);
...
function simPos({ attractorSize }) {
  ...
  uniform float delta;
    ...
    void main (void) {
      ...
      data_sim_velocity.xyz += normalize(rebornNormalData.rgb) * 1.3 * surfaceEmissionForce;
